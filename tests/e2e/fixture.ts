...
export const events = [
  ...
  event("alpha", "2026-10-02", "18:30", "Evening seminar on stars", { free: true, location: "Burlington House, Piccadilly, W1J 0BG" }),
  ...
  // Gamma failed today; this comes from its last good scrape.
  event("gamma", "2026-10-12", "18:00", "Gamma lecture from the last good run", { free: true }),
...
export const snapshots = health.sources
  .filter((s) => s.lastOkAt)
  .map((s) => ({ id: s.id, scrapedAt: s.lastOkAt, events: events.filter((e) => e.source === s.id) }));
